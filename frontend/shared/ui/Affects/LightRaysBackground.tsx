...
type OGLRenderingContext = WebGL2RenderingContext & {
  canvas: HTMLCanvasElement;
  renderer: Renderer;
};
type WebGLLoseContext = { loseContext(): void; restoreContext(): void };

const LightRaysBackground: React.FC<LightRaysProps> = ({
  ...
  useEffect(() => {
    ...
    const initializeWebGL = async () => {
      ...
      const program = new Program(gl as unknown as OGLRenderingContext, {
        vertex: vert,
        fragment: frag,
        uniforms,
      });
      ...
      const updatePlacement = () => {
        ...
        const dpr = renderer.dpr as number;
        const w = wCSS * dpr;
        const h = hCSS * dpr;
      ...
      const loop = (t: number) => {
        ...
        if (motionReduce) {
          uniforms.pulsating.value = 0.0;
          uniforms.raysSpeed.value = Math.min(raysSpeed, 0.6);
        }

        try {
          renderer.render({ scene: mesh });
          animationIdRef.current = globalThis.requestAnimationFrame(loop);
        } catch (error) {
      ...
      cleanupFunctionRef.current = () => {
        if (animationIdRef.current) {
          globalThis.cancelAnimationFrame(animationIdRef.current);
  ...
  useEffect(() => {
    ...
    const { clientWidth: wCSS, clientHeight: hCSS } = containerRef.current;
    const dpr = (renderer.dpr as number) || 1;
